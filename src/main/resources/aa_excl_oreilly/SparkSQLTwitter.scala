...
object SparkSQLTwitter {
  def main(args: Array[String]) {
    if (args.length < 2) {
      println(
        "Usage inputFile outputFile [spark.sql.inMemoryColumnarStorage.batchSize]"
      )
    }
    ...
    val happyPeopleRDD =
      sc.parallelize(List(HappyPerson("holden", "coffee"))).toDF()
    happyPeopleRDD.registerTempTable("happy_people")
    // UDF
    hiveCtx.udf.register("strLenScala", (_: String).length)
    val tweetLength =
      hiveCtx.sql("SELECT strLenScala('tweet') FROM tweets LIMIT 10")
    tweetLength.collect().map(println(_))
    // Two sums at once (crazy town!)
    val twoSums = hiveCtx.sql(
      "SELECT SUM(user.favouritesCount), SUM(retweetCount), user.id FROM tweets GROUP BY user.id LIMIT 10"
    )
