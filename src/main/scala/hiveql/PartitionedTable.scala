...
object PartitionedTable {
  ...
  def main(args: Array[String]) {
    ...
    val spark =
      SparkSession
        .builder()
        .appName("HiveQL-PartitionedTable")
        .master("local[4]")
        .enableHiveSupport()
        .getOrCreate()
    ...
    // the last columns. In HiveQL the partition columns are fully specified in the
    // PARTITION BY clause and omitted from the core column definition -- contrast
    // this with the analogous statement in sql/PartitionedTable.scala.
    //
    spark.sql(s"""
         | CREATE EXTERNAL TABLE partitioned
         |    (id INTEGER, cat INTEGER)
    ...
    // must be provided last! We need the partitions to be assigned
    // dynamically according to the data without specifying *any* of the
    // partition columns statically, so we need the non-strict setting.
    //
