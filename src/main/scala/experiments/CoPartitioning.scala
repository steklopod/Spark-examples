...
object CoPartitioning {
  ...
  def main(args: Array[String]) {
    ...
    val orders = Seq(Order(1001, 1, "A001", 100),
    ...
    spark.sparkContext
      .parallelize(orders, 2)
      .toDF()
      .createOrReplaceTempView("orders1")

    val res1 = spark.sql(
      "SELECT * FROM custs1 INNER JOIN orders1 ON custs1.id = orders1.custid")
