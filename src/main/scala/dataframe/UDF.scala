...
object UDF {
  ...
  def main(args: Array[String]) {
    ...
    val mySales = udf { (sales: Double, disc: Double) =>
      sales - disc
    }

    println("*** UDF used for selecting")
    customerDF
      .select($"id", mySales($"sales", $"discount").as("After Discount"))
      .show()
    ...
    def stateRegion = udf { (state: String) =>
      state match {
    ...
    customerDF
      .orderBy(stateRegion($"state"))
      .orderBy(mySales($"sales", $"discount"))
      .show()
    ...
    val salesFilter = udf { (s: Double, min: Double) =>
      s > min
    }
