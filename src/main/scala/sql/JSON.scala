...
object JSON {
  def main(args: Array[String]) {
    ...
    val young =
      spark.sql("SELECT firstName, lastName FROM people WHERE age < 30")
    ...
    val inPA = spark.sql(
      "SELECT firstName, lastName FROM peopleAddr WHERE address.state = 'PA'")
    ...
    val peopleAddrBad =
      spark.read.json("src/main/resources/data/notFlatBadFieldName.json")
    ...
    val lines =
      spark.read.textFile("src/main/resources/data/notFlatBadFieldName.json")
    ...
    val inPAFixed = spark.sql(
      "SELECT firstName, lastName FROM peopleAddrFixed WHERE address.state = 'PA'")
